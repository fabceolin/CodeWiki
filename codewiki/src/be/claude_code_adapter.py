...
# Override via CODEWIKI_MAX_PROMPT_TOKENS env var
import os as _os
DEFAULT_MAX_PROMPT_TOKENS = int(_os.environ.get("CODEWIKI_MAX_PROMPT_TOKENS", "730000"))
...
def _invoke_claude_code(
    ...
    # Truncate prompt if it exceeds the token limit instead of rejecting
    if prompt_tokens_estimate > max_prompt_tokens:
        max_chars = max_prompt_tokens * 4  # Reverse the ~4 chars/token estimate
        logger.warning(
            f"Prompt too large: ~{prompt_tokens_estimate:,} tokens estimated, "
            f"max allowed: {max_prompt_tokens:,} tokens. "
            f"Truncating from {prompt_chars:,} to {max_chars:,} chars."
        )
    ...
    try:
        ...
        # Retry with truncated prompt if Claude Code CLI reports "Prompt is too long"
        if result.returncode != 0 and "Prompt is too long" in (result.stdout or ""):
            truncated_chars = int(prompt_chars * 0.85)
            logger.warning(
                f"Claude Code CLI rejected prompt as too long. "
                f"Retrying with truncated prompt: {prompt_chars:,} → {truncated_chars:,} chars "
                f"(~{truncated_chars // 4:,} tokens)"
            )
            result = subprocess.run(
                cmd,
                input=prompt[:truncated_chars],
                capture_output=True,
                text=True,
                timeout=timeout,
                cwd=working_dir,
